...
class Big_window(): #Register
    ...
    #     """ Diviser la liste en 2 groupes WIN et LOSE """
        ...
        self.joueurV=[StringVar() for _ in range(4)]
    ...
    #     """----------------------------- END Controller things for players matchs ----------------------------------"""
        ...
        for index, joueurN in enumerate(PLAYERS_SELECT):
            ...
            if (index%2 == 0):
                ...
                textvariable=self.joueurV[index],
        ...
        validate_btn = Button(BtnF_Frame,text="Valider", command= lambda: self.validate_combo(self.joueurV,index), width="20",pady=5).grid(row=0, column=9, padx=10,pady=10)
    ...
    def validate_combo(self,joueurV,index):
            for i in range(4):
                dbTest1.insert({'points': (self.joueurV[i].get())})
