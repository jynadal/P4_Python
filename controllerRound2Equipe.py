...
dbTournament = TinyDB('dbRound2_P_listTest.json')
...
with open ('dbTournEquipeA.json','r') as myfileA:
    mydata=myfileA.read()
    objs = json.loads(mydata)
    last_T=(len(objs["_default"])) # the last tournament
    myObjsA = objs["_default"]['1']['equipe_A']

    #print(myObjsA)

    # for player in myObjsA:
    #     print(player)
...
round2_ListS_points = sorted(round2_List, key = lambda i: i['points'], reverse=True)
...
print(round2_ListS_points)

jsonList = json.dumps(round2_ListS_points)
jsonFile = open("dbRound2_P_list.json", "w")
