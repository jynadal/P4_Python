...
dbTest1 = TinyDB('dbTest1.json')
dbTest2 = TinyDB('dbTest2.json')
...
class Big_window(): #Register
    ...
    with open ('dbFinal_P_list.json','r') as mylist:
        data=mylist.read()
    ...
    #PLAYERS_WIN = [ d for d in R2_players if d['points'] > 0 ]   A SUPP
    #PLAYERS_LOSE = [ d for d in R2_players if d['points'] == 0 ]   A sUPP
    ...
    def __init__(self, window,title, geometry, bg_color, message):
       # personnaliser la fenêtre
        ...
        """ Match_lable """
        lbl_fMatch = Label(Outer_Frame, text="Final Match", bg=background_color2,fg=background_color,font=("lato",50,"bold"))
        lbl_fMatch.grid(row=0, column=0, pady=50, padx=100)
        lbl_sMatch = Label(Outer_Frame, text="Small Final Match", bg=background_color2,fg=background_color,font=("lato",50,"bold"))
        lbl_sMatch.grid(row=1, column=2, pady=00, padx=200)
    ...
    #     """ Diviser la liste en 2 groupes WIN et LOSE """
        ...
        self.joueur1A = StringVar()
        self.joueur1B = StringVar()
        self.joueur2A = StringVar()
        self.joueur2B = StringVar()

        # self.joueurN = [StringVar() for _ in range(2)]
    ...
    #     """----------------------------- END Controller things for players matchs ----------------------------------"""
        ...
        VS_Frame = Frame(Outer_Frame, bg=background_color)
        VS_Frame.grid(row=0, column=0, pady=40,padx=30)

        # VS_Frame.place(x=0,y=50,width=300,height=350)

        """ Afficher joueur1 des matches."""
        # for joueurA in round1_A:
        lbl_player1 = Label(VS_Frame, text=PLAYERS_SELECT[0]["fname"]+" "+PLAYERS_SELECT[0]["lname"], bg="black",fg="white",font=("lato",18,"bold"))
        lbl_player1.grid(row=0, column=0, pady=5,padx=30,sticky="N" )

        combo_player1 = ttk.Combobox(VS_Frame,
        textvariable=self.joueur1A, 
        font=("lato",13,"bold"),width=15, state='normal')
        combo_player1['values'] = ('Win','Lose')
        combo_player1.grid(row=1, column=0, pady=5, padx= 0)

        lbl_VS = Label(VS_Frame, text="VS", bg="blue",fg="black",font=("lato",25,"bold"))
        lbl_VS.grid(row=2, column=0)

        # ------------------------ 1 B -----------------------------------

        VS_Frame = Frame(Outer_Frame, bg="purple")
        VS_Frame.grid(row=1, column=0, pady=0,padx=50, sticky="S")
        
        combo_player2 = ttk.Combobox(VS_Frame,
        textvariable=self.joueur1B, 
        font=("lato",13,"bold"),width=15, justify="center", state='normal')
        combo_player2['values'] = ('Win','Lose')
        combo_player2.grid(row=1, column=0, pady=10, padx= 0)

        lbl_player2 = Label(VS_Frame, text=PLAYERS_SELECT[1]["fname"]+" "+PLAYERS_SELECT[1]["lname"], bg="white",fg="black",font=("lato",18,"bold"))
        lbl_player2.grid(row=0, column=0, pady=5,padx=30)


        # # ------------------------ 22222222 -----------------------------------

        VS_Frame = Frame(Outer_Frame, bg=background_color)
        VS_Frame.grid(row=0, column=1, pady=40,padx=30)

        # VS_Frame.place(x=0,y=50,width=300,height=350)

        """ Afficher joueur1 des matches."""
        # for joueurA in round1_A:
        lbl_player1 = Label(VS_Frame, text=PLAYERS_SELECT[2]["fname"]+" "+PLAYERS_SELECT[2]["lname"], bg="black",fg="white",font=("lato",18,"bold"))
        lbl_player1.grid(row=0, column=1, pady=5,padx=30,sticky="N" )

        combo_player1 = ttk.Combobox(VS_Frame,
        textvariable=self.joueur2A, 
        font=("lato",13,"bold"),width=15, state='normal')
        combo_player1['values'] = ('Win','Lose')
        combo_player1.grid(row=1, column=1, pady=5, padx= 0)
     ...
     # ------------------------ 2 B -----------------------------------
        ...
        VS_Frame = Frame(Outer_Frame, bg="purple")
        VS_Frame.grid(row=1, column=1, pady=0,padx=50, sticky="S")
        
        combo_player2 = ttk.Combobox(VS_Frame,
        textvariable=self.joueur2B, 
        font=("lato",13,"bold"),width=15, justify="center", state='normal')
        combo_player2['values'] = ('Win','Lose')
        combo_player2.grid(row=1, column=1, pady=10, padx= 0)

        lbl_player2 = Label(VS_Frame, text=PLAYERS_SELECT[3]["fname"]+" "+PLAYERS_SELECT[3]["lname"], bg="white",fg="black",font=("lato",18,"bold"))
        lbl_player2.grid(row=0, column=1, pady=5,padx=30)

       
        # self.b1 = Button(tk, text="Check", command=self.validate_combo).place(x=60, y=300)

        """ Btn First Frame"""
        BtnF_Frame = Frame(Outer_Frame,relief=RIDGE, bg=background_color2)
        BtnF_Frame.place(x=0,y=315,width=1500,height=50)

        validate_btn = Button(BtnF_Frame,text="Valider", command=self.validate_combo, width="20",pady=5).grid(row=0, column=9, padx=10,pady=10)
    ...
    def validate_combo(self):
        #for i in range(4):
        dbTest1.insert({'points1': (self.joueur1A.get()),'points2': (self.joueur1B.get()),'points3': (self.joueur2A.get()),'points4': (self.joueur2B.get())})
                # dbTest2.insert({'points': (joueurN[index].get())})
        print((self.joueur1A.get()))
        print((self.joueur2A.get()))
        print((self.joueur1B.get()))
        print((self.joueur2B.get()))
      


def Initialisation():
    window = Tk()
    Initialisation = Big_window(window,"Demi finale","1500x900+50+50","#2f435e","Demi-Finale")
    #window.mainloop()
